...
// Building classes
import { SmallIndigoPlant, LargeIndigoPlant, SmallSugarMill, LargeSugarMill, TobaccoStorage, CoffeeRoaster, } from '../../domain/buildings/catalog/ProductionBuildings';
import { SmallMarket, Smithy, Hacienda, Hospice, SmallWarehouse, Office, LargeMarket, LargeWarehouse, Factory, University, Harbour, Wharf, } from '../../domain/buildings/catalog/SmallUtilityBuildings';
import { Fortress, GuildHall, CustomsHouse, CityHall, Residence, } from '../../domain/buildings/catalog/LargeBuildings';
const STORAGE_KEY = 'puerto_rico_save';
// ── Building factory ──────────────────────────────────────────────────────────
const BUILDING_FACTORIES = {
...
// ── Serialization ─────────────────────────────────────────────────────────────
function savePlantation(p) {
    return { type: p.type, workers: p.occupiedWorkers };
}
function saveBuildingSlots(island) {
...
function savePlayer(p) {
    const goods = {};
    for (const [g, n] of p.storedGoods.entries())
        goods[g] = n;
    return {
        id: p.id,
        name: p.name,
...
function saveSupply(s) {
    const goods = {};
    for (const [g, n] of s.goodsPool.entries())
        goods[g] = n;
    return {
...
function buildSaveGame(state, playerSetups) {
    return {
        version: 1,
        savedAt: Date.now(),
...
// ── Deserialization ───────────────────────────────────────────────────────────
function restoreIsland(plantations, buildingSlotData) {
    const island = new Island();
    // Restore plantations preserving slot positions
    const restoredPlantations = plantations.map(pl => {
        if (!pl)
            return null;
        const p = new Plantation(pl.type);
    ...
    const restoredBuildings = buildingSlotData.map(data => {
        if (!data) {
            lastBuilding = null;
            return null;
        }
        if (data.shared && lastBuilding)
            return lastBuilding;
...
function restoreSupply(s) {
    ...
    for (const [g, n] of Object.entries(s.goods)) {
        supply.goodsPool.set(g, n);
    }
    supply.plantationDecks = s.decks.map(deck => deck.map(pl => {
        const p = new Plantation(pl.type);
        p.occupiedWorkers = pl.workers;
        return p;
    }));
...
function restoreGameState(s) {
    ...
    for (const good of s.tradingHouse) {
        if (good)
            tradingHouse.addGood(good);
    }
    const roleCards = s.roleCards.map(c => {
        const card = new RoleCard(c.type);
...
function restoreSetups(setups) {
    return setups.map(s => s.type === 'human'
...
// ── Public API ────────────────────────────────────────────────────────────────
export function serializeGame(state, playerSetups) {
    try {
...
//# sourceMappingURL=GameSerializer.js.map