...
const GOOD_META: Record<GoodType, { label: string; icon: string; bg: string; color: string }> = {
...
export function PlayerPanel({ player, setup, isActive, isGovernor, isSelector }: Props) {
  ...
  return (
    <div className={[
      ...
      <div className="player-panel__header">
        ...
        {isGovernor && <div className="governor-label">Gubernator</div>}
      </div>

      <div className="player-stats">
        <span className="stat" title="Dublony">🪙 <strong>{player.doubloons}</strong></span>
      ...
      {/* Island */}
      <IslandView island={player.island} />
    </div>
  );
}
