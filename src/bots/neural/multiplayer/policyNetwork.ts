// Isolated experimental copy of the verified Adam/MLP implementation.
// The distinct format prevents accidentally loading these weights in the released bot.
...
export interface NeuralPolicyJSON {
  format: 'puerto-rico-multiplayer-policy';
  version: 1;
...
export class NeuralPolicyNetwork {
  ...
  static fromJSON(value: unknown): NeuralPolicyNetwork {
    ...
    if (data.format !== 'puerto-rico-multiplayer-policy' || data.version !== 1 || data.featureSchema !== NEURAL_FEATURE_SCHEMA ||
  ...
  toJSON(metadata: Record<string, unknown> = this.metadata, includeOptimizer = false): NeuralPolicyJSON {
    const data: NeuralPolicyJSON = {
      format: 'puerto-rico-multiplayer-policy', version: 1, featureSchema: NEURAL_FEATURE_SCHEMA,
