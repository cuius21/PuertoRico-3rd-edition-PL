...
export interface NeuralModelJSON {
  format: 'puerto-rico-neural-value';
  version: 1 | 2;
...
export class NeuralValueNetwork {
  ...
  static fromJSON(value: unknown): NeuralValueNetwork {
    ...
    if (data.format !== 'puerto-rico-neural-value' || ![1, 2].includes(data.version) || data.playerCount !== 3 ||
  ...
  toJSON(metadata: Record<string, unknown> = this.metadata, includeOptimizer = false): NeuralModelJSON {
    const model: NeuralModelJSON = {
      format: 'puerto-rico-neural-value', version: this.valueMode === 'residual' ? 2 : 1,
      ...(this.valueMode === 'residual' ? { valueMode: this.valueMode } : {}), featureSchema: NEURAL_FEATURE_SCHEMA,
      featureNames: [...NEURAL_FEATURE_NAMES], playerCount: 3, hiddenSize: this.hiddenSize,
  ...
  private forward(inputs: number[][], baseline?: number[]): { probabilities: number[]; hidden: Float64Array } {
    if (inputs.length !== 3) throw new Error('Neural input requires three candidate rows');
    const hidden = new Float64Array(3 * this.hiddenSize);
    const logits = [0, 0, 0];
    if (this.valueMode === 'residual') {
      if (!baseline) throw new Error('Residual neural value requires baseline probabilities');
      this.validateTarget(baseline);
      for (let player = 0; player < 3; player++) logits[player] = Math.log(Math.max(1e-12, baseline[player]!));
    }
  ...
  loss(examples: readonly NeuralExample[]): number {
    ...
    for (const example of examples) {
      ...
      this.validateTarget(example.target);
      for (let player = 0; player < 3; player++) loss -= example.target[player]! * Math.log(Math.max(1e-12, probabilities[player]!));
    }
    return loss / examples.length;
  }

  private validateTarget(target: number[]): void {
    finiteArray(target, 3, 'target');
