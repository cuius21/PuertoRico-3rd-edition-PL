...
export const NEURAL_FEATURE_SCHEMA = 'puerto-rico-base3-public-v1';
...
const GLOBAL_NAMES = [
  ...
  ...ROLES.flatMap(role => [role + ':available', role + ':doubloons/6', role + ':owner0', role + ':owner1', role + ':owner2']),
...
export const NEURAL_FEATURE_NAMES = [
  ...[0, 1, 2].flatMap(seat => PLAYER_NAMES.map(name => 'seat' + seat + ':' + name)),
  ...GLOBAL_NAMES,
] as const;
...
function encodeRows(state: GameState, candidates: readonly number[]): number[][] {
  if (!supportsNeuralState(state)) throw new Error('Neural features support the base game with three players only');
  const players = state.players.map((player, index) => playerFeatures(state, player, index));
  return candidates.map(candidate => {
    const row = [0, 1, 2].flatMap(offset => players[(candidate + offset) % 3]!);
    ...
    for (const role of ROLES) {
      ...
      for (let offset = 0; offset < 3; offset++) row.push(Number(card?.takenBy === state.players[(candidate + offset) % 3]!.id));
    }
