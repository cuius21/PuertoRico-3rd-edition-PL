...
import { chooseChampionHeuristicAction, scoreChampionAction } from './championPolicy';
import { evaluateChampionState } from './championEvaluation';
...
export interface HardcoreOptions {
  ...
  selectionRule?: 'ucb' | 'puct';
  explorationConstant?: number;
  rolloutExploration?: number;
  coordinatedWorkers?: boolean;
  jointShipping?: boolean;
  rolloutPolicy?: 'heuristic' | 'neural';
}
...
function uniqueActions(actions: Action[]): Action[] {
  if (!actions.some(action => action.type === 'BUILD')) return actions;
  return [...new Map(actions.map(action => [hardcoreActionKey(action), action])).values()];
}
...
export class ChampionBot implements Bot {
  readonly name = 'ChampionBot';
  ...
  private readonly selectionRule: 'ucb' | 'puct';
  private readonly explorationConstant: number;
  private readonly rolloutExploration: number;
  private readonly coordinatedWorkers: boolean;
  private readonly jointShipping: boolean;
  private readonly rolloutPolicy: 'heuristic' | 'neural';
  ...
  constructor(options: HardcoreOptions = {}) {
    ...
    this.selectionRule = options.selectionRule ?? 'ucb';
    this.explorationConstant = options.explorationConstant ?? 1.4;
    this.rolloutExploration = options.rolloutExploration ?? 0.16;
    this.coordinatedWorkers = options.coordinatedWorkers ?? false;
    this.jointShipping = options.jointShipping ?? false;
    this.rolloutPolicy = options.rolloutPolicy ?? 'heuristic';
    if (!['ucb', 'puct'].includes(this.selectionRule) || !Number.isFinite(this.explorationConstant) ||
      this.explorationConstant < 0 || !Number.isFinite(this.rolloutExploration) || this.rolloutExploration < 0 || this.rolloutExploration > 1 ||
      !['heuristic', 'neural'].includes(this.rolloutPolicy)) {
      throw new Error('Invalid champion search configuration');
    }
  }
  ...
  private search(state: GameState, playerId: PlayerId): Action {
    const actions = uniqueActions(state.getValidActions(playerId));
    ...
    const fallback = this.heuristic(state, playerId, actions);
    ...
    while (this.lastSearchStats.iterations < this.maxIterations && performance.now() < deadline) {
      ...
      while (!sim.gameOver && steps++ < 1800) {
        ...
        const legal = depth === 0 ? actions : uniqueActions(sim.getValidActions(pid));
        if (legal.length === 0) break;
        if (depth > 0 && (legal.length === 1 || legal.some(a => a.type === 'PLACE_WORKER'))) {
          if (!sim.apply(this.heuristic(sim, pid, legal)).ok) break;
          continue;
        }
        ...
        for (let i = 0; i < legal.length; i++) {
          ...
          const value = this.selectionRule === 'puct' ?
            (child?.visits ? child.totals[mover]! / child.visits :
              (current.visits ? current.totals[mover]! / current.visits : 1 / sim.players.length) - 0.08) +
              this.explorationConstant * priors[i]! * Math.sqrt(current.visits + 1) / ((child?.visits ?? 0) + 1) :
            !child ? 100 + priors[i]! : child.totals[mover]! / Math.max(1, child.visits) +
  ...
  private priors(state: GameState, pid: PlayerId, actions: readonly Action[]): number[] {
    ...
    const scores = actions.map(a => this.jointShipping ? scoreChampionAction(a, state, pid) : scoreHardcoreAction(a, state, pid));
  ...
  private rollout(state: GameState, targetRound: number): void {
    for (let step = 0; step < 2000 && !state.gameOver; step++) {
      ...
      const actions = uniqueActions(state.getValidActions(pid));
      if (actions.length === 0) return;
      let action: Action;
      if (actions.length === 1 || actions.some(a => a.type === 'PLACE_WORKER')) {
        action = this.heuristic(state, pid, actions);
      } else if (this.random() > this.rolloutExploration) {
        if (this.rolloutPolicy === 'neural' && this.evaluator?.policy && !this.policyEvaluatorFailed) {
          const priors = this.priors(state, pid, actions);
          let selected = 0;
          for (let i = 1; i < priors.length; i++) if (priors[i]! > priors[selected]!) selected = i;
          action = actions[selected]!;
        } else action = this.heuristic(state, pid, actions);
      } else {
        const priors = this.priors(state, pid, actions);
    ...
    for (let i = 0; i < 500 && !state.gameOver && state.getCurrentPhase().type === PhaseType.Mayor; i++) {
      ...
      if (actions.length === 0 || !state.apply(this.heuristic(state, pid, actions)).ok) break;
    }
  }

  private evaluate(state: GameState): number[] {
    if (state.gameOver) return terminalUtilities(state);
    const base = this.jointShipping ? evaluateChampionState(state) : evaluateHardcoreState(state);
    if (this.evaluatorWeight === 0) return base;
    let learned: readonly number[] | undefined;
  ...
  private heuristic(state: GameState, playerId: PlayerId, actions: Action[]): Action {
    if (!this.coordinatedWorkers && !this.jointShipping) return chooseHeuristicAction(state, playerId, actions);
    return chooseChampionHeuristicAction(state, playerId, actions, { workers: this.coordinatedWorkers, shipping: this.jointShipping });
  }
}
